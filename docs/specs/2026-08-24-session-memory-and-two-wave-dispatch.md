...
## 16. Follow-on: a general question is not a lookup

Two disclosures found by reading real answers rather than by testing, both in
`tools/telemetry.py`, both the same mistake: a payload whose scope did not match
the scope of the question.

### 16.1 A global list reported as one patient's
...
The answer denies and confirms spo2 in consecutive sentences. `available_metrics`
was every metric in the store, returned beside a `detail` that said "that
patient" - so the model read the two as one statement. 12456 has no monitored
devices at all.

The hint now follows the question: asked about one patient, it lists only what
that patient has, and says plainly when that is nothing. A miss also stops
reporting what the rest of the population is monitored for.

A field added during the fix had to be removed again. `patient_has_devices:
false` was redundant next to `available_metrics: []`, and the model recited it:
*"the available metrics for this patient are empty because `patient_has_devices`
is false."* Fewer fields, less schema to leak into prose.

### 16.2 A configuration question answered with a person

    Q: What is the current alert threshold for SpO2?
    A: The threshold is 92%. This applies to device DEV-1398
       (patient_id: 13788), and their latest reading was 86%, which breached it.

No patient appears anywhere in that question. 13788 owned the first matching
device, and a question about a setting became a disclosure that a specific
person is hypoxaemic.

The split is now explicit: **a threshold is configuration, a reading is clinical
data.** A call with no `patient_id` returns thresholds, directions, units and a
device *count* - never a device id, never an owner, never a reading.
`evaluate_thresholds` refuses to evaluate breaches at all without a patient,
because a breach is a fact about a person.

Rows group by the settings rather than by metric, so a metric configured two
ways reports two rows instead of one confident wrong number.

This is the same line `fhir._unresolved` draws for an ambiguous name - how many,
never which. That module refused to enumerate the store on a miss; this was the
identical enumeration arriving through a hit, which is why it went unnoticed.

### 16.3 Verification

- `backend/tests/`: **211 passed**. `test_threshold_direction_is_honoured` now
  evaluates per patient, since an unscoped call no longer returns readings - the
  direction logic is just as checkable one chart at a time.
- Two new tests assert the boundary directly: no `DEV-` string and no patient id
  in an unscoped payload, and no `evaluations` key without a patient.
