...
By default everything runs locally on `qwen3.5:4b` via Ollama — no API keys, no
network calls, no data leaving the machine. One environment variable switches
the model to a hosted one so the whole thing can be deployed behind a link; the
PHI guardrail is what makes that swap defensible, and
[decision 25](docs/decisions.md) argues it out rather than waving at it.
...
`deploy` puts the key in Secret Manager, grants Cloud Run access to it, and
builds through Cloud Build — so Docker is never needed locally. The
[`Dockerfile`](Dockerfile) is one image: Node builds the UI, Python serves both
it and the API from a single origin, which is why there is no CORS
configuration in production.

Three flags carry reasoning rather than convention:

- **`--max-instances 1`** — session memory lives in the process
  ([decision 23](docs/decisions.md)). A second instance would strand half the
  conversations and break two-wave dispatch intermittently rather than visibly.
  This is the deployment's real scaling ceiling.
- **`--min-instances 0`** — what keeps it free. The first request pays a cold
  start, so load the URL once before demoing.
- **`--region us-central1`** — Cloud Run's free allowance only applies in a few
  US regions.

Why not host the model itself: GCP's Always Free VM is 1 GB of RAM against
weights that need four and a half, and there is no free GPU anywhere on the
platform. [Decision 25](docs/decisions.md) works through the alternatives,
